...
_Name_pair = [("scan", "index"), ("spectrum", "spectrum"), ("id", "id"),
              ("rt", "scan start time"), ("ms_level", "ms level"),
              ("precursormz", "selected ion m/z"), ("charge", "charge state"),
              ("collision", "collision"), ("energy", "energy")]


def _decode_binary(string, array_length,
                   precision=64,
    ...
    return unpack(f"<{array_length}{int_type}", decoded)
...
class _ParseSpectrumElement:
    ...
    def _get_spectrum(self, decode=_decode_binary):
        """
        Decode spectrum
        """
        ...
        for array in self._element.iter(tag="binaryDataArray"):
            ...
            if compress.startswith("no "):
                array_data = decode(binary.text, array_len,
                                    precision=bit, decompress=None)
            else:
                array_data = decode(binary.text, array_len, precision=bit)
    ...
    def _get_param(self, subelement):
...
class mzMLReader():
    ...
    def get_mass_spectrum(self, index=None, mslevel=None):
        """
        ...
        mslevel: int or None
                 Mass spectrum level in the experiment. If is
                 set to None, all mass spectra specified by
                 index are output, regardless of the level.
        """
    ...
    def _read_spectrum(self, fromstring=et.fromstring):
        ...
        for i0, i1 in zip(indices[:-1], indices[1:]):
            ...
            # corrects offsets to make sure spectrum bounds
